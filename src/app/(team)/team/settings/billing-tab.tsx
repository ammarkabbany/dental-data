      ...
      <motion.div variants={itemVariants}>
        <div className="mb-6">
          ...
          <p className="text-sm text-muted-foreground">
            You are currently on the <span className="font-medium text-foreground capitalize">{plan?.$id}</span> plan. 
            {plan?.$id !== "pro" && " You can upgrade your plan at any time to get more features."}
          </p>
        </div>
              ...
              <TableRow className="hover:bg-muted/30">
                <TableCell>
                  <div className="flex items-center gap-2">
                    <HugeiconsIcon icon={Pdf01Icon} className="text-red-500 size-6" />
                ...
                <TableCell className="hidden lg:table-cell">
                  <Badge variant="outline" className="font-normal">Starter Plan</Badge>
                </TableCell>
                <TableCell className="hidden lg:table-cell">
...
const PlanCard = ({ name, desc, price, features, isCurrent, upgradeOrDowngrade }: PlanCardProps) => {
  return (
    <Card className={`border h-full flex flex-col overflow-hidden transition-all duration-200 ${isCurrent ? 'border-primary shadow-md ring-1 ring-primary/20' : 'hover:border-primary/50 hover:shadow-sm'}`}>
      {isCurrent && (
