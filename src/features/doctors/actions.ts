...
export const UpdateDoctorDue = async (id: Doctor["$id"], due: number) => {
  const doctor = await GetDoctorById(id, [Query.select(["due"])]);
  const payload = {
...
export const GetDoctors = async (
  queries: string[] = [Query.limit(9999)]
): Promise<Doctor[]> => {
...
export const ListDoctors = async (teamId: string) => {
  ...
  return JSON.parse(response.responseBody)?.data as Doctor[];
